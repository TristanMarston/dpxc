...
const EditDocument = ({ data, id, collection }: { data: any; id: string; collection: string }) => {
    ...
    const [documentData, setDocumentData] = useState<any>(data);
    const preChanges: any = deepCopy(data);

    function deepCopy(obj: any): any {
        return Array.isArray(obj) ? obj.map(deepCopy) : obj !== null && typeof obj === 'object' ? Object.fromEntries(Object.entries(obj).map(([k, v]) => [k, deepCopy(v)])) : obj;
    }
    ...
    const submitChanges = (id: string) => {
        const submitData: any = {};
        schema.forEach(({ key, type, required }) => {
            if (documentData[key] !== preChanges[key]) {
                let typeMatch =
        ...
        axios
            .put(`/api/admin/put/${collection}/${id}`, JSON.stringify(submitData))
            .then((res) => {
                if (res.status === 200) {
    ...
    return (
        <div>
            <DialogTitle className='text-[1.35rem] phone:text-2xl text-background'>Edit Document</DialogTitle>
            <div className='w-full flex flex-col gap-2 mt-2'>
                {schema.map(({ key, type, required }: { key: string; type: string; required: boolean }, index: number) => {
                    return (
                        <div className='grid grid-rows-1 grid-cols-[100px_1fr] tablet:grid-cols-[112px_1fr]' key={key + type + 'modal'}>
...
const NestedObject = ({
    ...
    return (
        <div className='shadow-[0_3px_10px_rgb(0,0,0,0.1)] rounded-md text-sm max-h-[200px] overflow-y-scroll'>
            <span className='h-9 flex items-center justify-between p-2 gap-1.5'>
                <span className='flex gap-2'>
                    <p>
                        {object.length} document{object.length === 1 ? '' : 's'}
                    </p>
                    <CirclePlus
                        onClick={() => {
                            setOpened(true);
