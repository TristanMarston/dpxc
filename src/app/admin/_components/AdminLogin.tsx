	...
	return (
		<div className='mt-28 text-secondary w-full flex flex-col items-center gap-6'>
			<div className='text-center'>
				...
				<h3
					className={`${delaGothic.className} text-base mt-3 text-center font-bold text-secondary tracking-wide [text-shadow:_3px_3px_3px_rgba(0,0,0)]`}
				>
						...
						<label
							...
							className={`transform select-none cursor-text transition-all bg-background-light origin-[0] duration-300 absolute opacity-90 top-0 left-0 h-3/4 translate-y-1.5 flex items-center ml-3.5 text-sm ${
								usernameFocused || username.length > 0
									? '!text-[11px] !h-1/2 !-translate-y-2.5 !ml-2.5 !px-1 !opacity-100'
									: ''
							}`}
						>
							Username
					...
					<div className='w-full relative group'>
						...
						{passwordHidden ? (
							<EyeOff
								...
								className='absolute right-4 top-0 translate-y-2 text-secondary'
							/>
						) : (
							<Eye
								onClick={() => setPasswordHidden((prev) => !prev)}
